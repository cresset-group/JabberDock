...
# pow_loc = "~/POW_v2"
pow_loc = os.path.join(os.path.dirname(os.path.realpath(__file__)), "..", "POW_v2", "POW.py")
####################
...
if not restart:
    if not os.path.isdir("models"):
        ...
        os.rename(
            "models",
            "models_old_%d%d_%d_%d_%d" % (now.hour, now.minute, now.day, now.month, now.year),
        )
        # subprocess.call("mkdir models", shell=True)
        os.mkdir("./models")
        os.mkdir("./models/additional_files")
...
if mpi:
    ...
    #     shell=True,
    # )
    # run_docking.py script is generated by Flare
    # subprocess.call(["mpiexec", "-np", str(no_proc), sys.executable, "run_docking.py"])

    # Use this instead if running from command line
    subprocess.call(["mpiexec", "-np", str(no_proc), sys.executable, pow_loc, "input_ensemble"])
else:
    # subprocess.call(
    #     "%s/POW.py input_ensemble | tee time_log.dat" % (pow_loc), shell=True
    # )
    subprocess.call([sys.executable, "run_docking.py"])
