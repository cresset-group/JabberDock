...
if rank > -1:
    ...
    # initialize PSO optimizer separately in order to set random state later
    if optm.split(".")[0] == "PSO":
        import PSO
        ...
        exec(
            "from %s import %s as Optimizer" % (optm.split(".")[0], optm.split(".")[0])
        )
        search = Optimizer()
    ...
    # set random state for PSO
    if optm.split(".")[0] == "PSO":
        PSO.set_random_state(params.rng_file)
    ...
    if params.fit == "NA":
        ...
        try:
            ...
            print(
                "ERROR: user defined fitness function file should contain a class Fitness with a function evaluate(id,values_array) "
            )
