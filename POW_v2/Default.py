...
class Parser:  # this is imported in the file
    ...
    def check_standard_variables(self):
        ...
        if (
            len(self.high_input) != len(self.boundary_type)
            and self.boundary_type[0] != "NA"
        ):
            print(
                "ERROR: boundaryType length inconsistent with low and high boundary length!"
            )
            print(
                "ERROR: %s dimensions given, but %s needed!"
        ...
        if self.dimensions > -1 and self.high_input[0] != "NA":
            print(
                "ERROR: either dimensions or boundaryMin and boundaryMax variables should be used to defind space size!"
            )
            sys.exit(1)
...
class Postprocess:
    ...
    def select_solutions(self, params):
        ...
        filt = all_log[
            all_log[:, -1] < params.accept, 2 : len(all_log)
        ]  # first is selecting lines and second is column, boolean indexing, slicing
        #           print "len(all_log)"
        #           print len(all_log)
        #           print "filt"
        #           print filt
        ...
        if len(filt) == 0:
            print(
                "WARNING: No solution below desired threshold ("
                + str(params.accept)
                + "), returning best 10 solution instead"
            )
            pos = np.argsort(
                all_log[:, -1]
            )  # sort the array for the lowest fitness values, this arrays is just an index order, make a[pos] to get the right one
            self.filter_log = all_log[
                pos[0:10], 2 : len(all_log)
    ...
    def select_solutions_unranked(self, params):
        ...
        if len(self.filter_log) == 0:
            print(
                "WARNING: No solution below desired threshold, returning best 10 solution instead"
            )
        ...
        while True:
            ...
            while new_addition:
                ...
                for i in range(0, len(k), 1):
                    distance = np.sqrt(
                        np.dot(C[cnt - 1] - P[k[i]], C[cnt - 1] - P[k[i]])
                    )
    ...
    def align(self, m1, m2):
        ...
        E0 = np.sum(np.sum(m1 * m1, axis=0), axis=0) + np.sum(
            np.sum(m2 * m2, axis=0), axis=0
        )
