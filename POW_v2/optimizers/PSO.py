...
def set_random_state(rng_file):
    """
    Set the random state for NumPy's random number generator.
    This function either saves the current random state to a file or loads a random state from a file, depending on whether a file path is provided.
    Parameters:
    rng_file (str or None): The file path to load the random state from. If None, the current random state is saved to "random_state.pkl".
    Returns:
    None
    """
    # handle random seed file - if it is not provided, use the default seed and save it to a file
    if rng_file == "None" or rng_file is None:
        random_state = np.random.get_state()
        with open("random_state.pkl", "wb") as f:
            pickle.dump(random_state, f)
        print(">>> random seed saved to random_state.pkl")

    # if the seed file is provided, load the seed from the file
    else:
        with open(rng_file, "rb") as f:
            random_state = pickle.load(f)
        np.random.set_state(random_state)
        print(f">>> random seed loaded from {rng_file}")
...
class PSO:
    ...
    def check_keywords(self):
        ...
        if self.params.restart_freq > self.params.max_steps:
            print(
                "ERROR: restart frequency should be smaller than total number of steps!"
            )
            sys.exit(1)
        ...
        if (
            self.params.restart_load != "NA"
            and os.path.isfile(self.params.restart_load) != 1
        ):
        ...
        if self.params.inertia_max < self.params.inertia_min:
            print(
                "ERROR: maximal inertia is lower than minimal inertia (or one of these values are undefined)!"
            )
            sys.exit(1)

        # check neighborhood conditions
        if (
            self.params.neigh_type != "geographic"
            and self.params.neigh_type != "indexed"
        ):
    ...
    def launch(self, params, space, fitness):
        ...
        if rank == 0:
            ...
            if self.params.restart_load != "NA":
                print(
                    ">> recovering swarm state from file %s..."
                ...
                if os.path.isfile(self.params.restart_load) == 1:
                    ...
                    print(
                        "ERROR: restart file %s not found!" % self.params.restart_load
                    )
                    sys.exit(1)
                ...
                if os.path.isfile(self.params.output_file) == 1:
                    ...
                    shutil.copy2(
                        self.params.output_file, "%s.bkp" % self.params.output_file
                    )

                    fout_log = open(self.params.output_file, "w")
                    fin_log = open("%s.bkp" % self.params.output_file, "r")
                    totlines = (
                        self.params.n_particles * (swarm.repeat + 1) * (swarm.ts + 1)
                    )
                    ...
                    fin_log.close()
                    print(
                        ">> logging data in file %s, previous log backed up as %s.bkp"
                ...
                # if logfile is not found, create a new one
                    ...
                    print(
                        "WARNING: previous log file %s not found, starting a new one..."
                    )
                    fout_log = open(self.params.output_file, "w")

                print(
                    ">> optimization restored from repetition %s and step %s"
            ...
            # generate a random swarm otherwise
                ...
                swarm.seed(
                    self.params.n_particles, self.space
        ...
        for r in range(start_repeat, self.params.repeat, 1):
            ...
            for ts in range(start_ts, self.params.max_steps, 1):
                ...
                self.params.inertia = self.params.inertia_max - float(ts) / float(
                ...
                while p < self.params.n_particles:
                    ...
                    [part_pp, part_pv, part_pmp, part_pmv, part_cv, part_repel] = (
                        part.run()
                    )
                ...
                if rank == 0:
                    max_lines = int(
                        np.ceil(float(self.params.n_particles) / float(size))
                    )
                    ...
                    if self.params.repel:
                        ...
                        if cnt_rep > 0:
                            print(
                                ">>> added %s new repellers (tot. repellers: %s)"
                    ...
                    if (
                        self.params.restart_freq > 0
                        and np.mod(ts + 1, self.params.restart_freq) == 0
                    ):
                        print(
                            ">> saving swarm state at repetition %s, step %s..."
                            % (r + 1, ts + 1)
                        )
                        swarm.ts = ts
                        swarm.repeat = r
...
class particle:
    ...
    def run(self):
        ...
        self.test = np.logical_and(
            abs(self.local)
            > abs(
                self.swarm.part_max_pos[self.n] + self.space.cell_size - self.row_pos
            ),
            self.space.boundary_type == 0,
        )
        self.local[self.test] = (
            self.swarm.part_max_pos[self.n][self.test]
        ...
        self.local_max = (
            self.params.cn * np.random.rand(len(self.space.low)) * (self.local)
        )
        ...
        self.test = np.logical_and(
            abs(self.glob) > abs(self.best_pos + self.space.cell_size - self.row_pos),
            self.space.boundary_type == 0,
        )
        self.glob[self.test] = (
            self.best_pos[self.test]
            + self.space.cell_size[self.test]
            - self.row_pos[self.test]
        )
        self.test = np.logical_and(
        ...
        self.global_max = (
            self.params.cp * np.random.rand(len(self.space.low)) * (self.glob)
        )
        ...
        self.row_vel_new_tmp[self.test] = np.multiply(
            np.sign(self.row_vel_new_tmp[self.test]), self.space.cell_size[self.test]
        )
        ### flag, kick, reseed ###
        # update particle position according to velocity if speed is high enough (wrapping on boundary conditions)
        self.velocity = float(
            np.sqrt(np.dot(self.row_vel_new_tmp, self.row_vel_new_tmp))
        )
        ...
        if self.velocity > self.params.kar:
            ...
            [self.row_pos_new, self.row_vel_new] = self.space.check_boundaries(
                self.row_pos_new_tmp, self.row_vel_new_tmp
            )
        # if velocity is too low, launch the kick and reseed procedure
            ...
            if self.swarm.current_val[self.n] > self.params.accept:
                ...
                print(
                    ">>> kicking particle %s (fitness = %s)"
                    % (self.n, self.swarm.current_val[self.n])
                )
                self.row_vel_new_tmp = rand_set(
                    -self.space.cell_size, self.space.cell_size
                )
                self.row_pos_new_tmp = self.row_pos + self.row_vel_new_tmp
                [self.row_pos_new, self.row_vel_new] = self.space.check_boundaries(
                    self.row_pos_new_tmp, self.row_vel_new_tmp
            ...
            # random kick and reseed particles being almost motionless and having a good fitness (needless to check)
                ...
                print(
                    ">>> reseeding particle %s (fitness = %s)"
                    % (self.n, self.swarm.current_val[self.n])
                )
        ...
        if self.params.repel:
            ...
            self.bias_tmp = np.multiply(
                np.sign(self.bias[self.test]), self.space.cell_size[self.test]
            )
